...
int main(int argc, char* argv[]){
	...
	int read = 0;
	int readTotal = 0;
	while((read = fread(buff + readTotal, 1, 1024, fp)) > 0){
		readTotal += read;
		//MD5Update(&ctx, buff, read);
	}
	for(int i = 0; i < LOOPS; i++){
		MD5Init(&ctx);
		MD5Update(&ctx, buff, sz);
		MD5Final(&ctx);
		//for(int i = 0; i < 16; i++){
		//	printf("%02x", ctx.digest[i]);		
		//}
	}
	printf("%d MB processed", ((LOOPS * sz) / 1024)/1024);
	fclose(fp);
	return 0;
}
*/
...
void
MD5Transform(UINT4 *buf, UINT4 *input){
