...
PlatformScreen::PlatformScreen(void) {
    imgs[0] = 0;
    imgs[1] = 0;
    display = 0;
    width = 0;
...
void PlatformScreen::doScreenShot() {
	printf("PlatformScreen::doScreenShot\n");
    if (display == 0) {
    	printf("PlatformScreen::doScreenShot(calling init())\n");
        init();
    }
    if (cursor == 1) {
        cursor = 0;
    ...
    if (imgs[cursor] == NULL) {
    	printf("PlatformScreen::doScreenShot(xgetimage)\n");
        imgs[cursor] = XGetImage(display, root, 0, 0, width, height,
                XAllPlanes(), ZPixmap);
    } else {
    	printf("PlatformScreen::doScreenShot(xgetsubimage)\n");
        XGetSubImage(display, root, 0, 0, width, height, XAllPlanes(), ZPixmap,
                imgs[cursor], 0, 0);
    }
    printf("leaving PlatformScreen::doScreenShot\n");
}
bool PlatformScreen::updateMousePosition(int & x, int & y){
...
int PlatformScreen::init() {
    printf("PlatformScreen::init\n");
