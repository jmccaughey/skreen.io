...
{
  trace(LOG_NAME, "fillPluginFunctionTable\n");
  if(aNPPFuncs == NULL)
...
{
  trace(LOG_NAME, "NP_Initialize\n");
//  NPError rv = fillNetscapeFunctionTable(aNPNFuncs);
//  if(rv != NPERR_NO_ERROR)
//    return rv;

  NPError rv = fillPluginFunctionTable(aNPPFuncs);
  if(rv != NPERR_NO_ERROR)
    return rv;
...
NPError NP_GetValue(void *npp, NPPVariable aVariable, void *aValue)
{
	trace(LOG_NAME, "NP_GetValue\n");
