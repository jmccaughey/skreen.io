/* @(#)root/win32gdk:$Name:  $:$Id: gifencode.c,v 1.1 2011/10/15 04:36:30 ubuntu Exp $ */
...
#define put_byte(A) _output[Nbyte++] = (unsigned char)(A);
...
long GIFencode(int Width, int Height, int Ncol, 
  ...
  for (y=0; y<Height; y++) {
    //(*get_scline)(y, Width, ScLine);
    unsigned char* ScLine = scanlines[y];
    x     = 0;
    if (y == 0)
      Code  = ScLine[x++];
    while(x < Width) {
