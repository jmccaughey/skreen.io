...
import com.deskblast.client.RpcScreenScrapeDataIntermediate;
import com.deskblast.client.RpcScreenScrapeMetaData;
import com.deskblast.client.RpcScreenScrapeMetaDataIntermediate;
...
public class ScrapeStreamReader {
	...
	public static final long INTERMEDIATE_SEND_THRESHOLD = 
		Long.MAX_VALUE-1L
		//200L
		;
//	
//	public static ScreenScrapeDataChunk readChunk(DataInputStream in) throws IOException{
//		int sequence = in.readInt();
//		ScreenScrapeDataChunk ssdc = new ScreenScrapeDataChunk(sequence);
//		
//	}
//	
	public static void readStream(RpcBlasterInfo rpcBlasterInfo, DataInputStream in, RpcScrapeStreamHandler handler) throws IOException{
		...
		for(int i = 0; i < tileCount; i++){
			...
			if(i == 0 || (elapsed < INTERMEDIATE_SEND_THRESHOLD)){ //(length+8) <= available){
				...
				if(!bSentMetaData){
					bSentMetaData = true;
					handler.handleScrape(rpcBlasterInfo, new RpcScreenScrapeMetaDataIntermediate(
							pendingQueue.toArray(new RpcAbstractTileInfoSprite[0]), 
							width, height, mouseX, mouseY, bKeyFrame));					
				}else{
					handler.handleScrape(rpcBlasterInfo, new RpcScreenScrapeDataIntermediate(
							pendingQueue.toArray(new RpcAbstractTileInfoSprite[0])));
				}
				pendingQueue.clear();
		...
		if(pendingQueue.size() > 0 || !bSentMetaData){
			if(!bSentMetaData){
				bSentMetaData = true;
				handler.handleScrape(rpcBlasterInfo, new RpcScreenScrapeMetaData(
						pendingQueue.toArray(new RpcAbstractTileInfoSprite[0]), 
						width, height, mouseX, mouseY, bKeyFrame));					
			}else{
				handler.handleScrape(rpcBlasterInfo, new RpcScreenScrapeData(
						pendingQueue.toArray(new RpcAbstractTileInfoSprite[0])));
			}
			pendingQueue.clear();
		}
		log.debug("done reading scrape");
	}
